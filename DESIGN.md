...
colors:
  ...
  surface-soft: "#fff7f7"
  surface-card: "#ffe4e6"
  surface-cream-strong: "#fecdd3"
  surface-dark: "#1f0b12"
  surface-dark-elevated: "#2a1018"
  surface-dark-soft: "#200f18"
  on-primary: "#ffffff"
  on-dark: "#fff1f2"
  on-dark-soft: "#a09d96"
  accent-teal: "#5db8a6"
...
Claude.com is the warmest, most editorial interface in the AI-product category. The base atmosphere is a **tinted cream canvas** (`{colors.canvas}` — #fff1f2) — distinctly warm, deliberately not the cool gray-white that every other AI brand uses. Headlines run a **slab-serif display** ("Copernicus" / Tiempos Headline) at weight 400 with negative letter-spacing, paired with **StyreneB / Inter** body sans. The combination feels like a literary publication, not a SaaS marketing page.

Brand voltage comes from the **cream + red pairing** — red (`{colors.primary}` — #e11d48) is the signature Anthropic accent, used on every primary CTA, on the brand wordmark, and on full-bleed callout cards. The red is warm, slightly muted, never cyan/blue — a deliberate counter-positioning against OpenAI's cool slate, Google's saturated blue, and Microsoft's corporate cyan.
...
**Key Characteristics:**
- Warm cream canvas (`{colors.canvas}` — #fff1f2) with dark warm-ink text (`{colors.ink}` — #141413). The brand's defining color choice.
- Red primary CTA (`{colors.primary}` — #e11d48). Used scarcely on individual buttons, generously on full-bleed red callout cards.
...
- Dark navy product mockup cards (`{colors.surface-dark}` — #1f0b12) carrying code blocks, terminal panels, model comparison data — the brand shows the product chrome at scale rather than abstract marketing illustrations.
- Light cream feature cards (`{colors.surface-card}` — #ffe4e6) — slightly darker than canvas, used for content-driven feature explanations.
...
### Brand & Accent
- **Red / Primary** (`{colors.primary}` — #e11d48): The signature Anthropic warm red. Used on every primary CTA background, on full-bleed red callout cards, on the brand wordmark accent. The most-recognized Anthropic color outside of the spike-mark logo.
- **Red Active** (`{colors.primary-active}` — #be123c): The press / hover-darker variant.
- **Red Disabled** (`{colors.primary-disabled}` — #fecdd3): A desaturated cream-tinted disabled state.
...
### Surface
- **Canvas** (`{colors.canvas}` — #fff1f2): The default page floor. Tinted cream — warm, deliberately not pure white.
- **Surface Soft** (`{colors.surface-soft}` — #fff7f7): Section dividers, very-soft band backgrounds.
- **Surface Card** (`{colors.surface-card}` — #ffe4e6): Feature cards, content cards. One step darker than canvas.
- **Surface Cream Strong** (`{colors.surface-cream-strong}` — #fecdd3): A strongest-cream variant used on selected category tabs and emphasized section bands.
- **Surface Dark** (`{colors.surface-dark}` — #1f0b12): Code editor mockups, model showcase cards, footer. The dominant dark surface.
- **Surface Dark Elevated** (`{colors.surface-dark-elevated}` — #2a1018): Elevated cards inside dark bands (settings panels in mockups).
- **Surface Dark Soft** (`{colors.surface-dark-soft}` — #200f18): Slightly lighter dark, used for code block backgrounds inside larger dark cards.
- **Hairline** (`{colors.hairline}` — #fecdd3): The 1px border tone on cream surfaces. Same hex as `{colors.primary-disabled}` — borders feel like one elevation step rather than ink lines.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): Text on red buttons.
- **On Dark** (`{colors.on-dark}` — #fff1f2): Cream-tinted white used on dark surfaces (echoes the canvas tone).
...
- **Error** (`{colors.error}` — #e11d48): Validation errors.
...
- **Callout / CTA bands:** `{spacing.xxl}` (48px) inside red callout cards; 64px inside the larger dark CTA band.
...
- Some hero illustrations use simple line-art with red and dark-burgundy strokes on cream — minimal, hand-drawn-feeling, never photorealistic.
...
- Simple line-art illustrations with red + dark-burgundy strokes on the cream canvas
...
**`top-nav`** — Cream nav bar pinned to the top of every page. 64px tall, `{colors.canvas}` background. Carries the Anthropic spike-mark + "Claude" wordmark at left, primary horizontal menu (Product, Solutions, Use Cases, Pricing, Research, Company) center-left, right-side cluster with "Sign in" text-link, "Try Claude" `{component.button-primary}` (red). Menu items in `{typography.nav-link}` (StyreneB 14px / 500).

### Buttons

**`button-primary`** — The signature red CTA. Background `{colors.primary}` (#e11d48), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` darkens to `{colors.primary-active}` (#be123c).
...
**`button-secondary-on-dark`** — Used over `{colors.surface-dark}` cards. Background `{colors.surface-dark-elevated}` (#2a1018), text `{colors.on-dark}`. Stays dark — the system never inverts to a light secondary on dark surfaces.
...
**`text-link`** — Inline body links in `{colors.primary}` (the red). Underlined on press; the red inline link is one of the system's most distinctive small details.
...
**`hero-illustration-card`** — A larger card holding the hero's right-side artifact — sometimes a red-stroke line illustration on cream background, sometimes a dark code editor mockup. Background `{colors.canvas}` or `{colors.surface-dark}` depending on context, rounded `{rounded.xl}` (16px).

**`feature-card`** — Used in 3-up feature grids. Background `{colors.surface-card}` (#ffe4e6 — slightly darker cream), rounded `{rounded.lg}` (12px), internal padding `{spacing.xl}` (32px). Carries a small icon at top, an `{typography.title-md}` headline, and a body description in `{typography.body-md}`.
...
**`callout-card-red`** — A full-bleed red card carrying a major call-to-action. Background `{colors.primary}` (#e11d48), text `{colors.on-primary}` (white), rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). The red surface IS the voltage; the CTA inside uses an inverted button style (cream/canvas button on red).
...
**`text-input-focused`** — Focus state. Border thickens or shifts to `{colors.primary}` (red) for emphasis. Carries a 3px red-at-15%-alpha outer ring.
...
**`badge-red`** — Red-fill badge for "NEW", "BETA", featured highlights. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.caption-uppercase}` (12px / 500 / 1.5px tracking), rounded `{rounded.pill}`, padding 4px × 12px.
...
**`cta-band-red`** — A pre-footer "Try Claude" CTA card. Full-width red fill, white type, rounded `{rounded.lg}`, padding 64px. Carries an h2 in `{typography.display-sm}` (still serif!), a sub-line, and a cream-button CTA.
...
- Reserve `{colors.primary}` (red) for primary CTAs and full-bleed `{component.callout-card-red}` moments. Don't paint accent moments red elsewhere.
...
- Don't use cool blue or saturated cyan as a brand accent. The red is the brand voltage.
- Don't put red everywhere. The red is scarce on individual elements and generous only on full-bleed red callout cards.
- Don't use Inter for display headlines. The serif character is the brand voice.
- Don't repeat the same surface mode in two consecutive bands. The pacing alternates: cream → cream-card → dark-mockup → cream → red-callout → dark-footer.
...
6. Cream + red + dark burgundy is the trinity. Don't introduce a fourth surface tone (no purple cards, no green sections).
