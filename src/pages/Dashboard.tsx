...
function EngineTag({ engine }: { engine: string }) {
  const colors: Record<string, string> = {
    lualatex: "bg-indigo-50 text-indigo-600 border-indigo-200",
    pdflatex: "bg-violet-50 text-violet-600 border-violet-200",
    xelatex:  "bg-fuchsia-50 text-fuchsia-600 border-fuchsia-200",
  };
  return (
...
export default function Dashboard() {
  ...
  return (
    <div className="neo-root relative min-h-screen overflow-hidden">
      ...
      {/* Background ambience */}
      <div className="ambient-orb bg-indigo-500/10 w-[700px] h-[700px] top-[-15%] right-[-15%]" style={{ animationDelay: "0s" }} />
      <div className="ambient-orb bg-violet-500/8  w-[500px] h-[500px] bottom-[-10%] left-[-10%]" style={{ animationDelay: "-5s" }} />
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-10 fade-up">
              ...
              <div className="flex items-center gap-3 flex-wrap mb-1">
                ...
                {user?.role && (
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${user.role === "admin" ? "bg-indigo-500" : "bg-slate-400"}`} />
                    {user.role}
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <button onClick={() => navigate("/editor")}
            className="glass-panel rounded-2xl p-5 text-left hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 cursor-pointer group border border-indigo-100">
            <div className="flex items-start justify-between mb-4">
              <div className="w-9 h-9 rounded-xl bg-indigo-500/10 border border-indigo-200 flex items-center justify-center">
            ...
            <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-1">New document</p>
            <p className="text-slate-700 font-semibold text-[15px]">LaTeX Studio</p>
          </button>

          {/* File count */}
          <div className="glass-panel rounded-2xl p-5">
            <div className="w-9 h-9 rounded-xl bg-violet-500/10 border border-violet-200 flex items-center justify-center mb-4">
              <svg className="w-4.5 h-4.5 text-violet-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <p className="text-xs font-semibold text-violet-500 uppercase tracking-wider mb-1">Documents</p>
          ...
          <div className="glass-panel rounded-2xl p-5">
            ...
            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-1">Connection</p>
        ...
        <div className="glass-panel rounded-2xl p-6 mb-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-[15px] font-semibold text-slate-700">My Documents</h2>
          ...
          {!filesLoading && files.length === 0 && !filesError && (
            <div className="flex flex-col items-center justify-center py-14 gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm">Create your first LaTeX document to get started.</p>
              </div>
              <button onClick={() => navigate("/editor")}
                ...
                <div key={file.id}
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-white/30">
                    <button onClick={() => navigate(`/editor?id=${file.id}`)}
                      className="neo-btn neo-btn-soft flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg text-indigo-600 text-xs font-semibold">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                    ...
                    <button onClick={() => setPendingDelete(file)}
                      ...
                      title="Delete"
                      className="neo-btn w-8 h-8 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-500 border border-rose-200 flex items-center justify-center disabled:opacity-50">
                      {deletingId === file.id ? (
